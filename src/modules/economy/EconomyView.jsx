...
import { EconomyPlanner } from "./EconomyPlanner";
...
const emptyForm = (accounts) => ({ type: "deposit", accountId: accounts[0]?.id || "", fromAccountId: accounts[0]?.id || "", toAccountId: accounts[1]?.id || accounts[0]?.id || "", amount: "", note: "", category: "Övrigt", date: localISO() });
const emptyAccountForm = { name: "", kind: "spending", openingBalance: "", color: "#3ddc84" };
const accountKinds = { spending: "Disponibelt", buffer: "Buffert", savings: "Sparande", investment: "Investering" };

export function EconomyView({ state, onUpsertTransaction, onDeleteTransaction, onSaveAccount, onDeleteAccount, onCreateGoal, onOpenGoal, onMutate }) {
  ...
  const openAccountEditor = (account = null) => {
    ...
    setAccountForm(account ? { name: account.name, kind: account.kind || "spending", openingBalance: String(account.openingBalance || 0), color: account.color || "#3ddc84" } : emptyAccountForm);
  };
  ...
  const submit = (event) => {
    ...
    const transaction = {
      ...
      amount,
      note: form.note.trim(),
      category: form.category.trim() || "Övrigt",
      date: form.date,
  ...
  return (
    <div className="page">
      ...
      <EconomyPlanner state={state} onMutate={onMutate} />

      <section className="section">
        <div className="section-title"><span>TRANSAKTIONER</span><small>{filtered.length}/{data.transactions.length}</small>{selectedIds.length > 0 && <span className="bulk-actions"><button onClick={() => { data.transactions.filter((item) => selectedIds.includes(item.id)).forEach(onDeleteTransaction); setSelectedIds([]); }}><Icon name="trash" size={14} /> Ta bort {selectedIds.length}</button><button onClick={() => setSelectedIds([])}>Avmarkera</button></span>}</div>
        ...
        <div className="history-list">
          {filtered.map((transaction) => {
            ...
            return <div className={`history-row transaction-row ${selectedIds.includes(transaction.id) ? "selected" : ""}`} key={transaction.id}><input className="row-select" type="checkbox" aria-label={`Markera ${transaction.note || transactionLabels[transaction.type]}`} checked={selectedIds.includes(transaction.id)} onChange={() => setSelectedIds((current) => current.includes(transaction.id) ? current.filter((id) => id !== transaction.id) : [...current, transaction.id])} /><span className={`transaction-icon ${transaction.type}`}><Icon name={transaction.type === "deposit" ? "arrowDown" : transaction.type === "withdrawal" ? "arrowUp" : "swap"} size={15} /></span><div><strong>{transaction.note || transactionLabels[transaction.type] || "Äldre historik"}</strong><small>{accountNameFor(transaction)} · {transaction.category || "Övrigt"} · {transaction.date}</small></div><b className={transaction.type === "withdrawal" ? "negative" : transaction.type === "deposit" ? "positive" : ""}>{transaction.type === "transfer" ? money(transaction.amount) : `${signed > 0 ? "+" : ""}${money(signed)}`}</b><span className="transaction-actions"><button aria-label={`Duplicera ${transaction.note || transactionLabels[transaction.type]}`} onClick={() => onUpsertTransaction({ ...transaction, id: `tx-${crypto.randomUUID()}`, occurredAt: new Date().toISOString(), date: localISO() }, false)}><Icon name="plus" size={14} /></button><button aria-label={`Redigera ${transaction.note || transactionLabels[transaction.type]}`} onClick={() => openEditor(transaction)}><Icon name="edit" size={14} /></button><button aria-label={`Ta bort ${transaction.note || transactionLabels[transaction.type]}`} onClick={() => setDeleting(transaction)}><Icon name="trash" size={14} /></button></span></div>;
          })}
      ...
      {editor && <Modal title={editor === "new" ? "Ny transaktion" : "Redigera transaktion"} onClose={() => setEditor(null)}><form className="form-stack" onSubmit={submit}>
        ...
        <div className="field-grid"><label>Kategori<input value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })} placeholder="Mat, boende, sparande…" /></label><label>Anteckning<input value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} placeholder="Vad var detta?" /></label></div>
