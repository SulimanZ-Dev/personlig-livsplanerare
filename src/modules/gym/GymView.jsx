...
export function GymView({ data, onSave }) {
  const catalog = useMemo(() => [...new Set([...data.exerciseCatalog, ...data.workouts.flatMap((workout) => workout.exercises.map((exercise) => exercise.name))])].filter(Boolean), [data]);
