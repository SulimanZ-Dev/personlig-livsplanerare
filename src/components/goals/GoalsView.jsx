...
export function GoalsView({ state, onCreate, onEdit, onQuickUpdate, onChecklist, onArchive, onDelete, onPin, onMovePin }) {
  ...
  return (
    <div className="page">
      ...
      {deleting && <Modal title="Radera mål permanent?" onClose={() => setDeleting(null)}><div className="confirm-stack"><p><strong>{deleting.name}</strong>, dess {Object.values(state.goalEntries).filter((entry) => entry.goalId === deleting.id).length} progressposter och dashboardkopplingar tas bort.</p><p className="undo-note">Du kan ångra direkt efteråt, men målet hamnar inte i arkivet.</p><button className="danger-button" onClick={() => { onDelete(deleting); setDeleting(null); }}><Icon name="trash" size={17} /> Radera målet</button><button className="secondary-button" onClick={() => setDeleting(null)}>Avbryt</button></div></Modal>}
    </div>
  );
}
